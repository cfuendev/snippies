...
const parseCSSColorName = (str) => {
  ...
  throw new CSSValueParsingError(
    `The provided value '${str}' is not a supported CSS color name.`
  );
};

const parseTailwindColor = (str) => {
  if (
    Object.keys(tailwindColorsAsRGB).find(
      (twColor) => str.toLowerCase() === twColor
    )
  ) {
    return parseRGB(tailwindColorsAsRGB[str.toLowerCase()]);
  }
  throw new CSSValueParsingError(
    `The provided value '${str}' is not a Tailwind color.`
  );
};
...
module.exports = { parseCSSColorName, parseTailwindColor, parseHEX, parseRGB };