...
A module that exposes a function through which you can make any string look like it was said spoken by Kobeni Higashikata from Chainsaw Man. ~~Currently~~ Previously using it for a small personal project, but I'm pretty sure there's someone out there who wants to automate string kobenification.

The output is randomized and depends on the amount of words that the string passed to the main `kobeniStutterString()` function is made up of. First, a random word is "stuttered" to ensure that if the String consists of a single word, that word is "stuttered". In the case that the string is longer than a word, if the number of words is even, half of the remaining words will be "stuttered". If the number is odd, only a third of the words will be "stuttered". And these are picked at random, off course.
...
**NOTE:**
For now, this module doesn't support inserting ellipsis (...) into the original text, but that could be easily added later, if I come back to this util.